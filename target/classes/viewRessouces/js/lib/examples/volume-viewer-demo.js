    ...
    window.viewer = BrainBrowser.VolumeViewer.start("brainbrowser", function(viewer) {
        ...
        $("#volume-type").change(function() {
            ...
            if ($(this).val() === "functional") {
                ...
                viewer.loadVolume({
                    type: "minc",
                    header_url: "/resources/viewRessouces/js/lib/examples/models/functional.mnc.header",
                    raw_data_url: "/resources/viewRessouces/js/lib/examples/models/functional.mnc.raw",
                    template: {
                ...
                viewer.loadVolumes({
                    volumes: [
                        {
                            type: "minc",
                            header_url: "/resources/viewRessouces/js/lib/examples/models/t2.mnc.header",
                            raw_data_url: "/resources/viewRessouces/js/lib/examples/models/t2.mnc.raw",
                            template: {
                                element_id: "volume-ui-template",
                                viewer_insert_class: "volume-viewer-display"
                            }
        ...
        viewer.addEventListener("volumeuiloaded", function(container, volume, vol_id) {
            ...
            // Create labels to be submitted...
            container.find("#label-series-add-button").click(function() {
                var newLabelNode = '<div id="label-series-container-' + compteur + '" class="pull-left label-series-container">'+
                    '<input style="margin-right:10px;" type="checkbox"  id="label-series-checkbox-' + compteur + '">'+
                    'X:<input id="voxel-i-1" class="control-inputs">'+
                    'Y:<input id="voxel-j-1" class="control-inputs">'+
                    'Z:<input id="voxel-k-1" class="control-inputs">'+
                    'Value:<input id="label-series-count" class="control-inputs" value="1">'+
                    '</div>';

                container.find("#label-series-div").append(newLabelNode);
                compteur = compteur + 1;
            });

            container.find("#label-series-remove-button").click(function() {
                container.find("#label-series input:checkbox").each(function() {
                    if($(this).is(':checked')){
                        $(this).parent().remove();
                    }
                });
            ...
            //logic handles tag selected labels
            container.find("#label-series-tag-button").click(function() {
                container.find("#label-series input:checkbox").each(function() {
                    if($(this).is(':checked')){
                        $(this).next().val($("#voxel-i-" + vol_id).val());
                        $(this).next().next().val($("#voxel-j-" + vol_id).val());
                        $(this).next().next().next().val($("#voxel-k-" + vol_id).val());
                    }
                });
            });

            container.find("#zoom-in-btn").click(function() {
                var zoomType = container.find("#zoom-type").val();
                var volumeViewerDisplay =container.find(".volume-viewer-display > canvas");

                var zoomInWidth = viewer.defaultSliceHeight * 2;
                var zoomInHeight = viewer.defaultSliceHeight * 2;
                viewer.setPanelSize(zoomInWidth, zoomInHeight, { scale_image: true });
                switch(zoomType) {
                    case "sagittal":
                        volumeViewerDisplay.eq(0).show();
                        volumeViewerDisplay.eq(1).hide();
                        volumeViewerDisplay.eq(2).hide();
                        break;
                    case "coronal":
                        volumeViewerDisplay.eq(0).hide();
                        volumeViewerDisplay.eq(1).show();
                        volumeViewerDisplay.eq(2).hide();
                        break;
                    case "transverse":
                        volumeViewerDisplay.eq(0).hide();
                        volumeViewerDisplay.eq(1).hide();
                        volumeViewerDisplay.eq(2).show();
                        break;
                }
            });

            container.find("#clear-zoom-btn").click(function(){
                var volumeViewerDisplay =container.find(".volume-viewer-display > canvas");

                viewer.setPanelSize(viewer.defaultSliceHeight, viewer.defaultSliceHeight, { scale_image: true });
                volumeViewerDisplay.eq(0).show();
                volumeViewerDisplay.eq(1).show();
                volumeViewerDisplay.eq(2).show();
        ...
        ////////////////////////////////////////
        viewer.defaultSliceWidth  = $(window).width() / 5 + 100;
        viewer.defaultSliceHeight = $(window).width() / 5 + 100;
        ////////////////////////////////////////
        // Set the size of slice display panels.
        ////////////////////////////////////////
        viewer.setDefaultPanelSize(viewer.defaultSliceWidth, viewer.defaultSliceHeight);

  ...
  window.viewer = BrainBrowser.VolumeViewer.start("brainbrowser", function(viewer) {
    ...
    viewer.addEventListener("volumeuiloaded", function(container, volume, vol_id) {
      ...
      container.find(".slice-series-div").each(function() {
        ...
        container.find(".label-series-div").each(function() {

            var div = $(this);
            div.find("#label-series-button1").click(function() {
                $("#voxel-i-1").val($("#voxel-i-" + vol_id).val());

                $("#voxel-j-1").val($("#voxel-j-" + vol_id).val());

                $("#voxel-k-1").val($("#voxel-k-" + vol_id).val());

            });
            div.find("#label-series-button2").click(function() {

                $("#voxel-i-2").val($("#voxel-i-" + vol_id).val());

                $("#voxel-j-2").val($("#voxel-j-" + vol_id).val());

                $("#voxel-k-2").val($("#voxel-k-" + vol_id).val());

            });
            div.find("#label-series-button3").click(function() {

                $("#voxel-i-3").val($("#voxel-i-" + vol_id).val());

                $("#voxel-j-3").val($("#voxel-j-" + vol_id).val());

                $("#voxel-k-3").val($("#voxel-k-" + vol_id).val());
            });
        });
